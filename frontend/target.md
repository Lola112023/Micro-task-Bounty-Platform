...
- [x] 创建 target.md（本文件）
- [ ] 安装依赖：Element Plus、Vue Router 4、Pinia、Axios、Day.js
- [ ] 配置 Vite（路径别名 @）
...
- [ ] 创建 UserLayout.vue 和 AdminLayout.vue

### Phase 2：认证页面

- [ ] LoginView.vue（OAuth + 管理员登录Tab）

### Phase 3：用户端核心页面

- [ ] TaskHallView.vue（任务大厅、筛选、推荐）
- [ ] TaskDetailView.vue（多状态复合页、留言板）
- [ ] PublishTaskView.vue（发布任务表单）
- [ ] MyTasksView.vue（我的任务列表）
- [ ] MyApplicationsView.vue（我的申请）
- [ ] ProfileView.vue（个人中心，5个子标签）
...
- [ ] ReportModal、AppealModal、EvaluateModal
- [ ] AvatarModal、NicknameModal、AnnouncementModal
- [ ] RechargeModal、WithdrawModal、CreditRestoreModal

### Phase 5：管理员端页面

- [ ] AdminLoginView
- [ ] DashboardView（仪表盘、图表）
